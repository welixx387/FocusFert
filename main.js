...
/* ------------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------------ */
const CRYPTOBOT_API_KEY = process.env.CRYPTOBOT_API_KEY || '622342:AAtMELFYOMaiB0HfGbH6FQifq73JGBUphRk';
const CRYPTOBOT_API_BASE = 'https://pay.crypt.bot/api'; // официальный эндпоинт Crypto Pay API
const ASSET = 'USDT';

/* Безопасно достаёт URL аватара: если поле avatar по какой-то причине не
   Parse.File (битые/старые данные у части аккаунтов), не роняем всю функцию
   (лидерборд, поиск, список друзей и т.д.), а просто отдаём null. */
function safeAvatarUrl(u) {
  try {
    const f = u.get('avatar');
    return (f && typeof f.url === 'function') ? f.url() : null;
  } catch (e) {
    return null;
  }
}
...
Parse.Cloud.define('createCryptoInvoice', async (request) => {
  ...
  /* --------------------------------------------------------------------
     ВАЖНО: без этой проверки функция создавала НОВЫЙ счёт в CryptoBot при
     каждом вызове — стоило закрыть и снова открыть модалку оплаты (или
     просто ещё раз кликнуть кнопку), как плодился очередной неоплаченный
     инвойс, и так бесконечно. Теперь сначала смотрим, нет ли уже "живого"
     pending-счёта у пользователя, и если он всё ещё активен в CryptoBot —
     отдаём его повторно вместо создания нового.
     -------------------------------------------------------------------- */
  const pendingQ = new Parse.Query(Invoice);
  pendingQ.equalTo('user', user);
  pendingQ.equalTo('status', 'pending');
  pendingQ.descending('createdAt');
  const pendingInvoices = await pendingQ.find({ useMasterKey: true });

  if (pendingInvoices.length) {
    const ids = pendingInvoices.map(inv => inv.get('invoiceId')).filter(Boolean).join(',');
    const checkRes = ids ? await Parse.Cloud.httpRequest({
      method: 'GET',
      url: `${CRYPTOBOT_API_BASE}/getInvoices?invoice_ids=${encodeURIComponent(ids)}`,
      headers: { 'Crypto-Pay-API-Token': CRYPTOBOT_API_KEY }
    }).catch(() => null) : null;

    const items = (checkRes && checkRes.data && checkRes.data.ok && checkRes.data.result && checkRes.data.result.items) || [];
    const byId = new Map(items.map(it => [String(it.invoice_id), it]));

    for (const inv of pendingInvoices) {
      const item = byId.get(inv.get('invoiceId'));
      if (item && item.status === 'active') {
        // Счёт ещё ждёт оплаты и не просрочен — переиспользуем его.
        return {
          invoiceId: inv.get('invoiceId'),
          payUrl: item.pay_url || item.bot_invoice_url
        };
      }
      // Просрочен / не найден / уже оплачен где-то ещё — помечаем и идём дальше.
  ...
  const body = {
    ...
    expires_in: 3600 // счёт сам истечёт в CryptoBot через час, если не оплачен
  };
...
Parse.Cloud.define('getFriendsData', async (request) => {
  ...
  const requests = pending.map(r => ({
    ...
    fromUser: {
      ...
      avatarUrl: safeAvatarUrl(r.get('fromUser')),
